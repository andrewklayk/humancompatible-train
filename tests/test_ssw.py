...
class TestSSG(unittest.TestCase):
    def setUp(self):
        ...
        self.optimizer = SSG(self.params, m=self.m, lr=0.01, dual_lr=0.01)
    ...
    def test_step_obj(self):
        ...
        for p in self.params:
            p.grad = torch.ones_like(p)
            self.optimizer.state[p]["c_grad"] = [-1.0 * torch.ones_like(p)]
            p_pre_step[p] = p.detach().clone()

        self.optimizer.step(c_val=torch.tensor(-1.0))

        for p in self.params:
            self.assertTrue(torch.all(p == p_pre_step[p] - 0.01 * torch.ones_like(p)))
