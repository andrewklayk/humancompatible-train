...
def run(cfg: DictConfig) -> None:
    ...
    for RUN_IDX in range(1, N_RUNS + 1):
        ...
        ## prepare files ##
        run_save_path = os.path.abspath(os.path.join(exp_save_path, str(RUN_IDX)))
        ...
        for iteration, state_dict in history["params"]["w"].items():
            # save model
            torch.save(
                state_dict, os.path.join(model_checkpoints_save_path, f"{iteration}.pt")
            )
        ...
        save_states_index = (
            alg_states_df.index % cfg.alg.params.save_state_interval == 0
        )

        if save_train:
            # TODO: join with table of iters and times based on checkpoint save interval
            table_train = run_summary_full_set(
                model=net,
                dataset=train_ds,
                path=model_checkpoints_save_path,
                constraints=c,
        ...
        if save_val:
            X_val_tensor, y_val_tensor = (
                tensor(X_val, dtype=DTYPE),
                tensor(y_val, dtype=DTYPE),
            )
            val_ds = TensorDataset(X_val_tensor, y_val_tensor)
            c_val = create_constraint_from_cfg(
                cfg,
                dataset=val_ds,
                group_indices=group_ind_val,
                loss_fn=torch.nn.BCEWithLogitsLoss(),
                device=device,
            )

            table_val = run_summary_full_set(
                model=net,
                dataset=val_ds,
                path=model_checkpoints_save_path,
                constraints=c_val,
        ...
        if save_test:
            ...
            c_test = create_constraint_from_cfg(
                cfg,
                dataset=test_ds,
                group_indices=group_ind_test,
                loss_fn=torch.nn.BCEWithLogitsLoss(),
                device=device,
            )

            table_test = run_summary_full_set(
                model=net,
                dataset=test_ds,
                path=model_checkpoints_save_path,
                constraints=c_test,
