...
def u_0(x):
    if type == 1:
        ...
        return 6./(tf.math.cosh(x)**2)
    

def u_0_x(x):
    if type == 1:
        return -np.pi*tf.math.sin(np.pi * x)
    elif type == 2:
        return -12.*tf.math.sinh(x)/(tf.math.cosh(x)**3)


def periodic_bc(model, x, t):
    xL = tf.ones_like(x) * xMin
    xR = tf.ones_like(x) * xMax
    uL = model(tf.concat([xL, t], axis=1))
    uR = model(tf.concat([xR, t], axis=1))
    return tf.reduce_mean((uL - uR)**2)


def V(u):
    return alpha*tf.pow(u, 3)/3 + rho*tf.pow(u, 2)/2


def kdv_density(u, u_x):
    return V(u)-nu*tf.pow(u_x, 2)/2
...
def PINNModel(num_hidden_layers=depth, num_neurons_per_layer=width):  # 8,40
    ...
    for _ in range(num_hidden_layers):
        output_u = tf.keras.layers.Dense(num_neurons_per_layer,
                                         activation=sigmoid_centered,  # mish
...
def grad_L2_fft_batch(r, L):
    ...
    k_pos = tf.range(0, Nx//2 + 1, dtype=tf.float32)
    k_neg = tf.range(-Nx//2 + 1, 0, dtype=tf.float32)
    k = tf.concat([k_pos, k_neg], axis=0)
    k = (2.0 * tf.constant(np.pi) / L) * k
    k = tf.cast(k, tf.complex64)

    r_hat = tf.signal.fft(r)

    grad_energy = tf.reduce_sum(tf.abs(1j * k * r_hat)**2, axis=-1)

    dx = L / tf.cast(Nx, tf.float32)
    return tf.math.real(grad_energy) * dx
...
def H1_norm_fft_batch(r, L):
    ...
    r_hat = tf.signal.fft(r)
    weight = 1.0 + tf.abs(k)**2

    H1_sq = tf.reduce_sum(weight * tf.abs(r_hat)**2, axis=-1)

    dx = L / tf.cast(Nx, tf.float32)
    return tf.math.real(H1_sq) * dx
...
def custom_loss(inputs, model, epoch):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        with tf.GradientTape(persistent=True) as tape2:            
            ...
            with tf.GradientTape(persistent=True) as tape3:
                tape3.watch(x)
                tape3.watch(t)
                u_model = model(tf.concat([x, t], axis=1))
    ...
    # === PDE residual loss (stabilized, consistent) ===
    pde_loss_L2 = tf.reduce_mean(tf.square(r))

    r_grid = tf.reshape(r, [Nt, Nx])
    L = xMax - xMin

    pde_loss_grad = tf.reduce_mean(
        grad_L2_fft_batch(r_grid, L)
    )

    # mesh-scaled stabilization parameter
    lam = 0.01 * (dx**2) * tf.minimum(1.0, epoch / 1000.0)
    ...
    #     lambdas
    # )
    # loss, loss_type = augmentedChebyshev_loss_function(
    #     [pde_loss_H1, data_fitting_loss_0, data_fitting_loss_l_r],
    #     lambdas
    # )
...
optimizer = tf.keras.optimizers.AdamW(learning_rate=learning_rate, beta_1=0.8, beta_2=0.9, epsilon=1e-07)
...
for epoch in range(epochs):
    if not stop:
        ...
        with tf.GradientTape() as tape:
            loss, loss_type, pde_loss, data_fitting_loss_0, data_fitting_loss_l_r, H_loss = custom_loss(inputs, model, epoch)
...
df.to_csv('./results/kdv/training_history.csv', index=False)
