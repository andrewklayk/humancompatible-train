...
def loss_equality(preds: Tensor, sens: Tensor, labels: Tensor, loss: Module, diff_to_overall: bool = False):
    """
    ...
    for group in range(sens.shape[-1]):
        ...
        group_losses[group] = loss(group_preds.squeeze(), group_labels)
        
    if not diff_to_overall:
        return torch.abs(group_losses[0] - group_losses[1])
    
    overall_loss = loss(preds.squeeze(), labels)
