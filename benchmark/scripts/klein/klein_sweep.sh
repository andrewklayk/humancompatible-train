...
#SBATCH --output=out/klein/sweep.out
#SBATCH --nodes=1
...
python -u PDE_Klein_Gordon.py -m algorithm=adam \
...
python -u PDE_Klein_Gordon.py -m algorithm=pbm \
...
python -u PDE_Klein_Gordon.py -m algorithm=alm \
    algorithm.primal__lr=0.0005,0.001,0.005 \
    +algorithm.primal__weight_decay=0.01 \
