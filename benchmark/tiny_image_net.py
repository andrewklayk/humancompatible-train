...
def dataset_to_tensors(dataset, batch_size=512, num_workers=2):
...
def load_or_cache(dataset, cache_path, batch_size=512, num_workers=2):
...
def train_tinyimagenet():
    ...
    for name, (X, targets) in raw_splits.items():
        ...
        loaders[name] = torch.utils.data.DataLoader(
            dataset_torch, batch_size=batch_size, shuffle=True, num_workers=2
        )
        loaders[name + "_balanced"] = torch.utils.data.DataLoader(
            dataset_torch, batch_sampler=sampler, num_workers=2
        )
