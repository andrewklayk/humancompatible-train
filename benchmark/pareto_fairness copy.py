...
# ── training loops ───────────────────────────────────────────────────────────
def _make_epoch_row(epoch, tr_batch_losses, tr_batch_viols, te_loss, te_viols, val_loss, val_viols, wall_time):
    ...
    row["val_viol_max"] = max(val_viols)
    for i in range(N_CONSTRAINTS):
        row[f"val_viol_{i}"] = val_viols[i]
    return row


def train_regularized(X_tr, y_tr, g_tr, X_val, y_val, g_val, X_te, y_te, g_te, lam, seed):
    model     = make_model(X_tr.shape[1], seed).to(device)
    ...
    init_val_loss, init_val_viols = evaluate(model, X_val, y_val, g_val)
    epoch_log.append(_make_epoch_row(0, [init_tr_loss], [init_tr_viols],
                                     init_te_loss, init_te_viols, init_val_loss, init_val_viols, 0.0))
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        val_loss_ep, val_viols_ep = evaluate(model, X_val, y_val, g_val)
        epoch_log.append(_make_epoch_row(epoch, tr_batch_losses, tr_batch_viols,
                                         te_loss_ep, te_viols_ep, val_loss_ep, val_viols_ep, wall_time))
...
LOG_EPOCH_FIELDS = (
    ...
    + ["val_loss", "val_viol_max"]
    + [f"val_viol_{i}" for i in range(N_CONSTRAINTS)]
)
...
def _best_epoch_test_metrics(method_name, hp, log_dir, viol_threshold):
    """
    Per seed: pick the epoch with the lowest val_loss where val_viol_max <= viol_threshold.
    Falls back to the minimum val_loss epoch if none satisfies the constraint.
    Returns a dict of test loss/violation mean+std across seeds, or None if logs are missing.
    """
    per_seed_te_loss, per_seed_te_viols = [], []
    for seed in SEEDS:
        ...
        try:
            ...
            return None
        feasible = [r for r in rows if r["val_viol_max"] <= viol_threshold]
        best = min(feasible or rows, key=lambda r: r["val_loss"])
        per_seed_te_loss.append(best["te_loss"])
        per_seed_te_viols.append([best[f"te_viol_{i}"] for i in range(N_CONSTRAINTS)])

    te_arr = np.array(per_seed_te_viols)
    return {
        "loss_mean":     np.mean(per_seed_te_loss),
        "loss_std":      np.std(per_seed_te_loss),
        "fair_max_mean": np.mean(te_arr.max(axis=1)),
        "fair_max_std":  np.std(te_arr.max(axis=1)),
    }


def plot(all_series, out="pareto_fairness.svg",
         use_best_epoch=False, log_dir=None, viol_slack=0.):
    """
    use_best_epoch: if True, replace final-epoch test metrics with best-epoch test metrics.
      For each (method, hp, seed), the best epoch is the one with the lowest val_loss
      subject to val_viol_max <= hp * (1 + viol_slack).  Falls back to min val_loss epoch
      when no epoch satisfies the constraint (e.g. eps=0 equality runs).
    """
    if use_best_epoch and log_dir is not None:
        processed = []
        for results, label, color, marker in all_series:
            ...
            for r in results:
                hp        = r["hp"]
                threshold = 0.05 * (1 + viol_slack)
                best      = _best_epoch_test_metrics(method_name, hp, log_dir, threshold)
                nr = dict(r)
                if best is not None:
                    nr.update(best)
                new_results.append(nr)
            processed.append((new_results, label, color, marker))
    else:
        processed = all_series
    ...
    _draw_pareto(ax_te, processed,
                 "loss_mean", "fair_max_mean", "loss_std", "fair_max_std",
                 "Test set (best val epoch)" if use_best_epoch else "Test set")
...
def plot_convergence(all_series, log_dir="benchmark_results/epoch_logs", out="convergence.svg",
    ...
    """
    Each (method, hp) combination gets its own column.
    ...
    # Build flat list of columns: one entry per (method, hp) combination
    columns = []
    for results, label, base_color, _marker in all_series:
        method_name = label.split(" (")[0]
        hparams = [r["hp"] for r in results]
        if hparams_filter is not None:
            allowed = (hparams_filter.get(method_name)
                       if isinstance(hparams_filter, dict) else hparams_filter)
            if allowed is not None:
                hparams = [hp for hp in hparams if hp in allowed]
        for hp in hparams:
            columns.append((method_name, hp, base_color))
    ...
    # Loss row: share y across all columns
    for col in range(1, n_cols):
        axes[0, col].sharey(axes[0, 0])

    # All violation rows: share a single y-axis
    viol_ref = axes[1, 0]
    ...
    for col, (method_name, hp, _base_color) in enumerate(columns):
        ...
        for row, (tr_key, te_key, ylabel) in enumerate(metrics):
            ...
            if data is not None:
                ...
                for key, color in [(tr_key, "tab:blue"), (te_key, "tab:orange")]:
                    ...
                    lbl  = "Test" if color == "tab:orange" else "Train"
            ...
            if row == 0:
                # hp_name = "bound" if method_name == "Regularization" else hp_name = "penalty"
                title = f"{method_name}\nhp={hp:.3g}"
                if method_name == "Regularization" and hp == 0:
                    title = "Unconstrained AdamW"
                else:
                    title = method_name
...
if __name__ == "__main__":
    ...
    plot(all_series, use_best_epoch=False, log_dir=log_dir)
