...
def _metric_traj(spec, method, cfg, metric):
    ...
    if metric == "objective":
        ...
        t = metric_trajectory(spec, method, cfg, "opt", "loss")
...
def _finals_by_method(spec, methods, metric, tail, feas_tol=None):
    d = {}
    for m in methods:
        if metric == "objective" and m == "adam":
            feas_tol = None
...
def _plot_cdf(ax, finals, log, ls):
    for m, rows in finals.items():
        st = style_for(m)
        vals = np.sort([r[1] for r in rows])
        y = np.arange(1, len(vals) + 1) / len(vals)
        ax.plot(vals, y, label=st["label"], color=st["color"], ls=ls,
...
def _plot_conv(ax, finals, log, ls):
    for m, rows in finals.items():
        st = style_for(m)
        best = min(rows, key=lambda r: r[1])
        for cfg, _final, traj, epochs in rows:
            ax.plot(epochs, traj, color=st["color"], alpha=0.12, lw=0.6,ls=ls)
...
def plot_kkt(spec, methods=None, mode="cdf", metric="residual", tail=5,
    ...
    if mode == "all":
        ...
        n_axes = len(mode) if not isinstance(mode, str) else 1
        fig, ax = plt.subplots(n_axes, 1, figsize=(COL_WIDTH * n_axes, COL_WIDTH * 0.8))
        if n_axes == 1:
            ax = [ax]

    rlabel = _AXLABEL.get(metric, metric)

    if isinstance(feas_tol, float):
        feas_tol = [feas_tol]

    for f, tol in enumerate(feas_tol): 
        ...
        feas = tol if metric == "objective" else None
        finals = _finals_by_method(spec, methods, metric, tail, feas_tol=feas)

        if not finals:
            extra = f" feasible at max_viol<={tol}" if feas is not None else ""
            print(f"no '{SPLIT}' metrics found under {spec.agg_root}{extra} "
                f"(run aggregate.py --approach opt first)")
            return None

        if feas is not None:
            print(f"[objective] keeping only configs with final max_viol <= {tol}")
        ...
        if mode == "all":
            # fig, axes = plt.subplots(2, 2, figsize=(COL_WIDTH * 2, COL_WIDTH * 1.7))
            _plot_cdf(axes[0, 0], finals, log, ls = linestyles[f]);      axes[0, 0].set_xlabel(rlabel)
            ...
            _plot_conv(axes[1, 1], finals, log, ls = linestyles[f]);     axes[1, 1].set_ylabel(rlabel)
        else:
            # n_axes = len(mode) if not isinstance(mode, str) else 1
            # fig, ax = plt.subplots(n_axes, 1, figsize=(COL_WIDTH * n_axes, COL_WIDTH * 0.8))
            if isinstance(mode, str):
                mode = [mode]
            for i, md in enumerate(mode):
                if md == "cdf":
                    _plot_cdf(ax[i], finals, log, ls = linestyles[f]); ax[i].set_xlabel(rlabel)
                elif md == "pdf":
                    _plot_pdf(ax[i], finals, metric, log); ax[i].set_xlabel(rlabel)
                elif md == "scatter":
                    _plot_scatter(ax[i], finals, log); ax[i].set_ylabel(rlabel)
                elif md == "conv":
                    _plot_conv(ax[i], finals, log, ls = linestyles[f]); ax[i].set_ylabel(rlabel)
                else:
                    raise ValueError(f"unknown mode '{md}'")
                plt.legend()
...
if __name__ == "__main__":
    ...
    for spec in specs: 
        ...
        for metric in ["residual", "grad_norm", "max_viol", "compl", "objective"]:
            # if metric == "objective":
            tol = [0.0, spec.bound/10]
            # else:
            #     tol = 0.0
            # plot_kkt(spec, out=out + f"{mapping_name[spec.task]}_{metric}.pdf", 
            #         mode=["cdf","conv"], metric=metric, feas_tol=tol)
