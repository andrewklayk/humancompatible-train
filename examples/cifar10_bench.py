...
def benchmark(n_epochs, n_constraints, seeds, savepath, dataloader_train, dataloader_test, threshold, classes, class_ind, mu, method_f):
    ...
    for idx, seed in enumerate(seeds):
        ...
        losses_cur, constraints_cur, losses_cur_t, constraints_cur_t = method_f(seed, n_epochs, dataloader_train, dataloader_test, threshold, mu)
...
def load_data(balanced=False):
    ...
    if balanced:
        ...
        trainloader = torch.utils.data.DataLoader(dataset_train, batch_size=64, num_workers=10)
    ...
    if balanced:
        ...
        testloader = torch.utils.data.DataLoader(dataset_test, batch_size=64, num_workers=10)
...
def adam(seed_n, n_epochs, dataloader_train, dataloader_test, fair_crit_bound, _):
    ...
    for lr in lrs:
            ...
            cur_fit = (S_loss_log_plotting[-1] + test_S_loss_log_plotting[-1]) / 2
    ...
    S_loss_log_plotting, S_c_log_plotting, test_S_loss_log_plotting, test_S_c_log_plotting  
...
if __name__ == '__main__':
    ...
    # log path file
    log_path = "./data/logs/log_benchmark_stochastic_cifar10_bench.npz"

    # load data
    trainloader, testloader, classes, class_ind = load_data(balanced=False)
    ...
    # # # benchmark pbm
    # benchmark(n_epochs, n_constraints, seeds, log_path, dataloader_train, dataloader_test, features_train, threshold, mu, pbm)
    # print('PBM DONE!!!')
